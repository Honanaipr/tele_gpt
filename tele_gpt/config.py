...
class AppConfig:
    ...
    class RedisConfig:
        url = environ.var(
            help="Redis url", validator=optional(matches_re(REDIS_URL_RE)), default=None
        )
        host = environ.var(
            help="Redis host", validator=optional(matches_re(HOSTNAME_RE)), default=None
        )
        port = environ.var(
            help="Redis port", validator=optional(matches_re(PORT_RE)), default=None
        )
    @environ.config(prefix="OPENAI")
    class OpenAiConfig:
        api_key = environ.var(
            help="OpenAI token", default=None
        )

    bot = environ.group(BotConfig)
    redis = environ.group(RedisConfig)
    open_ai= environ.group(OpenAiConfig)
